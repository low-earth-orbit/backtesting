...
class RentVsBuyCalculator:
    ...
    def simulate_rent(self, leverage_ratio: float = 1.0) -> pd.DataFrame:
        ...
        4. Optional leverage with margin loans (DYNAMIC REBALANCING):
           ...
           - leverage_ratio=1.38: Maintain 35% borrowed (1.38x invested) - rebalance annually
        ...
        for year in range(self.years):
            ...
            if leverage_ratio > 1.0 and margin_debt > 0:
                ...
                annual_margin_interest = margin_debt * self.margin_rate
    ...
    def print_comparison(
        ...
        rent_margin_138_final = rent_margin_138.iloc[-1]
        ...
        print(f"  Margin Rate: {self.margin_rate*100:.2f}%")
        ...
        # Scenario 3: Rent with 1.38x Margin
        print("\n" + "=" * 110)
        print("SCENARIO 3: RENT (with 1.38x margin leverage)")
        print("=" * 110)
        print(
            f"Total Rent Paid (25 years): ${rent_margin_138['annual_rent_paid'].sum():,.0f}"
        )
        print(
            f"Final Annual Rent: ${self.initial_annual_rent * (1 + self.annual_rent_increase) ** self.years:,.0f}"
        )
        print(
            f"Total Excess Invested (25 years): ${rent_margin_138['excess_available_for_investing'].sum():,.0f}"
        )
        print(
            f"Total Amount Invested (with 1.38x leverage): ${rent_margin_138['excess_available_for_investing'].sum() * 1.38:,.0f}"
        )
        print(f"Final Margin Debt: ${rent_margin_138_final['margin_debt']:,.0f}")
        print(
            f"Investment Portfolio: ${rent_margin_138_final['investment_portfolio']:,.0f}"
        )
        print(
            f"Net Liquid Wealth (Portfolio - Margin Debt): ${rent_margin_138_final['total_liquid_wealth']:,.0f}"
        )
        print(
            f"Total Margin Interest Paid: ${rent_margin_138['annual_margin_interest'].sum():,.0f}"
        )
        print(
            f"Total Margin Interest Tax Benefit: ${rent_margin_138['annual_margin_interest_tax_benefit'].sum():,.0f}"
        )
        print(f"\nBreakdown:")
        print(
            f"  Starting investment (down payment equivalent): ${self.down_payment:,.0f}"
        ...
        print(
            f"  Cumulative excess invested (35% borrowed): +${rent_margin_138['excess_available_for_investing'].sum() * 1.38:,.0f}"
        )
        print(
            f"  Investment growth over 25 years: +${rent_margin_138_final['investment_portfolio'] - (self.down_payment * 1.38) - (rent_margin_138['excess_available_for_investing'].sum() * 1.38):,.0f}"
        )
        print(
            f"  Margin interest (net of tax benefit): ${rent_margin_138['annual_margin_interest'].sum() - rent_margin_138['annual_margin_interest_tax_benefit'].sum():,.0f}"
        )
        ...
        rent_margin_138_advantage = (
            rent_margin_138_final["total_liquid_wealth"]
            - rent_final["total_liquid_wealth"]
        )
        print(
            f"{'3. Rent (1.38x margin)':<45} ${rent_margin_138_final['total_liquid_wealth']:>23,.0f} +${rent_margin_138_advantage:>22,.0f}"
        )
    ...
    def plot_networth(
        ...
        - Rent (1.38x margin): Investment portfolio - margin debt
        ...
        for idx in range(len(rent)):
            ...
            # RENT (1.38x margin): Portfolio - margin debt
            rent_margin_138_nw = (
                rent_margin_138.iloc[idx]["investment_portfolio"]
                - rent_margin_138.iloc[idx]["margin_debt"]
            )
            rent_margin_138_networth.append(rent_margin_138_nw)
        ...
        plt.text(
            years[final_year_idx],
            rent_margin_138_networth[final_year_idx],
            f"${rent_margin_138_networth[final_year_idx]/1e6:.2f}M",
            fontsize=8,
            va="bottom",
            ha="right",
            color="#F18F01",
...
def main():
    ...
    rent_margin_138 = calculator.simulate_rent(leverage_ratio=1.38)
    ...
    rent_margin_138.to_csv(
        "/Users/leohong/backtesting/home-ownership/scenario_rent_margin_138.csv",
        index=False,
    )
